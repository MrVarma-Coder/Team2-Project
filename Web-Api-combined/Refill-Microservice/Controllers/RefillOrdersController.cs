...
namespace RefillApi.Controllers
{
    ...
    {
        private readonly RefillContext _Db;
   ...
   */
        ...
        RefillOrder a = new RefillOrder();
        IRefillOrderProvider _refillOrderProvider;
        public RefillOrdersController(IRefillOrderProvider refillOrderProvider, RefillContext Db)
        {
            _Db = Db;
       ...
       [HttpGet("{id}")]
        public IActionResult RefillStatus(int id)
        {
            if (id >0)
            {
                ...
                var result = _Db.TbRefillOrder.Find(id);
                return Ok(result);
            }
            else
            {
        ...
        public IActionResult RefillDues(int id)
        {
            if (id > 0)
            {
                ...
                RefillOrder result = _Db.TbRefillOrder.Find(id);
                if (result == null)
                {
                    return Ok(a);
                }
                return Ok(result);
                // if (result.Id.Equals(null)) { return Ok(false); }
                /*   try
                   {
                       if (Convert.ToBoolean(result.Id)) { return Ok(result.Payment); }
                       return Ok("no");
                   }
                   catch( NullReferenceException) { return Ok(false); }
               }*/
            }
            else
            {
